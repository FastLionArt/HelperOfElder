...
&НаСервере
  Процедура ОбработкаКомандыСервер() Экспорт
    Адрес = "http://192.168.90.136/helper/ws/ws1.1cws?wsdl";
    Определения = Новый WSОпределения(Адрес);
    URI = "http://192.168.0.28/helper/";
    Прокси = Новый WSПрокси(Определения, URI, "WebСинхронизация", "WebСинхронизацияSoap");
